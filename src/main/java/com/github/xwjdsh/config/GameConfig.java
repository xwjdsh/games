...
public class GameConfig {
	 ...
	 * 排行数据路径
	 */
	private final String chartsPath;
	 ...
	 * 描述与键位的对应
	 */
	private Map<String, Integer> descriptionKeyMap;

	/**
	 * 描述与方法的对应
	 */
	private Map<String, String> descriptionMethodMap;
	...
	public void init_keys() {
		try {
			...
			// 初始化方法-键位映射
			this.methodKeyMap = new HashMap<>(keylist.size());
			// 初始化描述-键位映射
			this.descriptionKeyMap = new HashMap<>(keylist.size());
			// 初始化描述-方法映射
			this.descriptionMethodMap = new HashMap<>(keylist.size());
			...
			for (Element ele : keylist) {
				...
				descriptionKeyMap.put(describe, keycode);
				descriptionMethodMap.put(describe, methodName);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void updateKey(Map<String, Integer> newDestributeKeyMap) {
		// 改变当前描述与键位的引用
		this.descriptionKeyMap = newDestributeKeyMap;
		// 改变方法与键位的映射
		for (Map.Entry<String, Integer> entry : newDestributeKeyMap.entrySet()) {
			this.methodKeyMap.put(descriptionMethodMap.get(entry.getKey()), entry.getValue());
		}
	}
	...
	public void writeKeyConfig() {
		...
		try {
			...
			for (Map.Entry<String, Integer> entry : this.descriptionKeyMap.entrySet()) {
				// 通过xpath来获取对应描述的键位属性
			...
			xw = new XMLWriter(new FileOutputStream(this.cfgPath), format);
			// 写入xml文件
			xw.write(doc);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void updateCharts(Player player) {
		if (this.players == null) {
			this.players = new ArrayList<>(this.chartsNum + 1);
		}
		// 添加对象
	...
	private void writeCharts() {
		...
		try {
			...
			String path = Paths.get(resource.toURI()).toFile().getAbsolutePath();
			oos = new ObjectOutputStream(new FileOutputStream(path + "/" + this.chartsPath));
			// 输出
