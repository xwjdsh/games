...
public class SysConfig {
	 ...
	 * 单例对象
	 */
	private static final SysConfig cfg=new SysConfig();
	...
	private void init_game(Element element) {
		//默认记录的排行数
		int chartsNum = Integer.parseInt(element.attributeValue("chartsNum"));
		//读取并解析游戏的配置元素
		...
		for(int i=0;i<games.size();i++){
			...
			gameMap.put(i+1, new GameConfig(gameId,gameName, logicClassName, cfgPath, materialPath,dtoClassName,chartsPath, chartsNum));
		}
	}
